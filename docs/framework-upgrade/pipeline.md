...
4. `internal_call=true` 的请求 **禁止** 再跑任何增强阶段（视觉、记忆检索、记忆抽取）。最大嵌套深度 1。
...
| `VISION_COMPOSE_ENABLED` | 视觉阶段。还要求总开关为 true |
...
| 1 | 0 | 0 | 0 | 空流水线（计时字典可空）；S5 合成模型有图仍 fail-closed |
| 1 | 1 | 0 | 0 | 只视觉 |
| 1 | 0 | 1 | 0 | 只记忆检索 |
| 1 | 1 | 1 | 0 | 视觉 → 记忆检索 |
...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `model_group` | `str` | 客户端看见的逻辑模型（合成模型为 `glm-5.2-vision`） |
...
1. 若 `not is_gateway_enhance_enabled()` 或 `env.internal_call`：return（`stage_ms` 保持空）。
...
信封 `internal_call=true` 时跳过全部阶段。子调用 id 形态由 `vision-compose.md` / `memory.md` 规定：`{parent}#vision:{hash8}`、`{parent}#memory-extract:{hash8}`。
...
2. 不得留下半剥的 image block：关视觉后，合成模型要么从 discovery 消失，要么带图明确失败。
...
- [ ] flag 默认关时 `run_pipeline` 不改 messages、`stage_ms == {}`
- [ ] `internal_call=true` 同上
- [ ] `async_get_available_deployment` 在 select 之后调用 runner（可用 spy/计数证明）
- [ ] `GATEWAY_ENHANCE_ENABLED=false` 时 `tests/unit` + `tests/contract` 全绿
- [ ] 未改 Fill First / 同组 1 次 / 跨组 3 次
